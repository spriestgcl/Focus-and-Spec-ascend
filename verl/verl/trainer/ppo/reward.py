...
def compute_reward(data: DataProto, reward_fn: AbstractRewardManager) -> tuple[torch.Tensor, dict[str, Any]]:
    ...
    dummy_mask = data.non_tensor_batch.get("redundant_partial_dummy_mask")
    if dummy_mask is not None:
        dummy_mask_tensor = torch.as_tensor(dummy_mask, device=reward_tensor.device, dtype=torch.bool)
        if dummy_mask_tensor.any():
            reward_tensor = reward_tensor.clone()
            reward_tensor[dummy_mask_tensor] = 0
