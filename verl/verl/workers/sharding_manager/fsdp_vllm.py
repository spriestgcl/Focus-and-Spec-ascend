...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(
        ...
        should_hijack_lora = bool(
            os.getenv("VERL_ENABLE_VLLM_LORA_HIJACK", "0") == "1"
            or getattr(self.model_config, "lora_rank", 0) > 0
            or getattr(self.model_config, "lora_adapter_path", None) is not None
        )
        if is_version_ge(pkg="vllm", minver="0.7.3") and should_hijack_lora:
            VLLMHijack.hijack()
