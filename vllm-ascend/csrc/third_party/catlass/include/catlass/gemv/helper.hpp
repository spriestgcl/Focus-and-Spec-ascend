...
template <class Element> struct UBAlignHelper {
    static constexpr uint32_t ALIGN = BYTE_PER_BLK / sizeof(Element);
};

template <class GmAType> struct AtomicAddSelector {
    static_assert(DEPENDENT_FALSE<GmAType>, "Unsupported layout selector, can not find the specialization.");
};

template <class Element> struct AtomicAddSelector<Gemm::GemmType<Element, layout::RowMajor>> {
    static constexpr bool value = false;
};

template <class Element> struct AtomicAddSelector<Gemm::GemmType<Element, layout::ColumnMajor>> {
...
template <class Element> struct L1AlignHelper<Element, layout::VectorLayout> {
    static constexpr uint32_t ELE_NUM_PER_C0 = BYTE_PER_C0 / sizeof(Element);
    static constexpr uint32_t M_ALIGNED = C0_NUM_PER_FRACTAL;
    static constexpr uint32_t N_ALIGNED = ELE_NUM_PER_C0;
};
...
template <class GmAType, class GmBType> struct L1AndL0TypeSelectorGemv {
    static_assert(DEPENDENT_FALSE<GmAType>, "Unsupported layout selector, can not find the specialization.");
