...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['a', 'w1', 'w2', 'expertIdx', 'scale1', 'scale2', 'probs']
    output_names = ['out']
...
def _build_args(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M, transB, weightNz):
    __inputs__ = []
    for arg in [a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__]:
        if arg != None:
...
@tbe_register.register_operator("DispatchFFNCombine", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def dispatch_ffn_combine(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M=0, transB=False, weightNz=False, kernel_name="dispatch_ffn_combine", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M, transB, weightNz)
    ...
    custom_compile_options = {'__ALLSOC__': ['--cce-auto-sync=on', '-Wno-deprecated-declarations', '-Werror', '-DHCCL_COMM', '-I/usr/local/Ascend/ascend-toolkit/latest/aarch64-linux/ascendc/include', '-I/root/Focus-and-Spec-ascend/vllm-ascend/csrc/third_party/catlass/include']},
    ...
    origin_func_name = "dispatch_ffn_combine"
    ascendc_src_dir_ex = "dispatch_ffn_combine"
    ascendc_src_dir = "dispatch_f_f_n_combine"
    ascendc_src_file = "dispatch_ffn_combine.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator DispatchFFNCombine, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DispatchFFNCombine"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__], origin_outputs = [out_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0"], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M=0, transB=False, weightNz=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M, transB, weightNz)
    result = check_op_cap("op_select_format", "DispatchFFNCombine", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M=0, transB=False, weightNz=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M, transB, weightNz)
    result = check_op_cap("get_op_specific_info", "DispatchFFNCombine", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
