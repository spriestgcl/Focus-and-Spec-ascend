...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'expert_ids', 'gmm1_permuted_weight', 'gmm1_permuted_weight_scale', 'gmm2_weight', 'gmm2_weight_scale', 'expert_scales', 'expert_smooth_scales', 'x_active_mask']
    output_names = ['output', 'ep_recv_count']
...
def _build_args(x_in__, expert_ids_in__, gmm1_permuted_weight_in__, gmm1_permuted_weight_scale_in__, gmm2_weight_in__, gmm2_weight_scale_in__, expert_scales_in__, expert_smooth_scales_in__, x_active_mask_in__, output_out_, ep_recv_count_out_, group_ep, ep_rank_size, ep_rank_id, moe_expert_num, share_expert_num, share_expert_rank_num, quant_mode, global_bs):
    __inputs__ = []
    for arg in [x_in__, expert_ids_in__, gmm1_permuted_weight_in__, gmm1_permuted_weight_scale_in__, gmm2_weight_in__, gmm2_weight_scale_in__, expert_scales_in__, expert_smooth_scales_in__, x_active_mask_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [output_out_, ep_recv_count_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if ep_rank_size != None:
        attr = {}
        attr["name"] = "ep_rank_size"
        attr["dtype"] = "int"
        attr["value"] = ep_rank_size
        __attrs__.append(attr)
    if ep_rank_id != None:
        attr = {}
        attr["name"] = "ep_rank_id"
        attr["dtype"] = "int"
        attr["value"] = ep_rank_id
        __attrs__.append(attr)
    if moe_expert_num != None:
        attr = {}
        attr["name"] = "moe_expert_num"
        attr["dtype"] = "int"
        attr["value"] = moe_expert_num
        __attrs__.append(attr)
    if share_expert_num != None:
        attr = {}
        attr["name"] = "share_expert_num"
        attr["dtype"] = "int"
        attr["value"] = share_expert_num
        __attrs__.append(attr)
    if share_expert_rank_num != None:
        attr = {}
        attr["name"] = "share_expert_rank_num"
        attr["dtype"] = "int"
        attr["value"] = share_expert_rank_num
        __attrs__.append(attr)
    if quant_mode != None:
        attr = {}
    ...
    if global_bs != None:
        ...
        attr["name"] = "global_bs"
        attr["dtype"] = "int"
        attr["value"] = global_bs
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("DispatchGmmCombineDecode", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def dispatch_gmm_combine_decode(x_in__, expert_ids_in__, gmm1_permuted_weight_in__, gmm1_permuted_weight_scale_in__, gmm2_weight_in__, gmm2_weight_scale_in__, expert_scales_in__, expert_smooth_scales_in__=None, x_active_mask_in__=None, output_out_=None, ep_recv_count_out_=None, group_ep="", ep_rank_size=0, ep_rank_id=0, moe_expert_num=0, share_expert_num=0, share_expert_rank_num=0, quant_mode=0, global_bs=0, kernel_name="dispatch_gmm_combine_decode", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, expert_ids_in__, gmm1_permuted_weight_in__, gmm1_permuted_weight_scale_in__, gmm2_weight_in__, gmm2_weight_scale_in__, expert_scales_in__, expert_smooth_scales_in__, x_active_mask_in__, output_out_, ep_recv_count_out_, group_ep, ep_rank_size, ep_rank_id, moe_expert_num, share_expert_num, share_expert_rank_num, quant_mode, global_bs)
    ...
    custom_compile_options = {'__ALLSOC__': ['--cce-auto-sync=off', '-Wno-deprecated-declarations', '-Werror', '-I/root/Focus-and-Spec-ascend/vllm-ascend/csrc/third_party/catlass/include']},
    ...
    origin_func_name = "dispatch_gmm_combine_decode"
    ascendc_src_dir_ex = "dispatch_gmm_combine_decode"
    ascendc_src_dir = "dispatch_gmm_combine_decode"
    ascendc_src_file = "dispatch_gmm_combine_decode.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator DispatchGmmCombineDecode, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DispatchGmmCombineDecode"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, expert_ids_in__, gmm1_permuted_weight_in__, gmm1_permuted_weight_scale_in__, gmm2_weight_in__, gmm2_weight_scale_in__, expert_scales_in__, expert_smooth_scales_in__, x_active_mask_in__], origin_outputs = [output_out_, ep_recv_count_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0"], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(x_in__, expert_ids_in__, gmm1_permuted_weight_in__, gmm1_permuted_weight_scale_in__, gmm2_weight_in__, gmm2_weight_scale_in__, expert_scales_in__, expert_smooth_scales_in__=None, x_active_mask_in__=None, output_out_=None, ep_recv_count_out_=None, group_ep="", ep_rank_size=0, ep_rank_id=0, moe_expert_num=0, share_expert_num=0, share_expert_rank_num=0, quant_mode=0, global_bs=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, expert_ids_in__, gmm1_permuted_weight_in__, gmm1_permuted_weight_scale_in__, gmm2_weight_in__, gmm2_weight_scale_in__, expert_scales_in__, expert_smooth_scales_in__, x_active_mask_in__, output_out_, ep_recv_count_out_, group_ep, ep_rank_size, ep_rank_id, moe_expert_num, share_expert_num, share_expert_rank_num, quant_mode, global_bs)
    result = check_op_cap("op_select_format", "DispatchGmmCombineDecode", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, expert_ids_in__, gmm1_permuted_weight_in__, gmm1_permuted_weight_scale_in__, gmm2_weight_in__, gmm2_weight_scale_in__, expert_scales_in__, expert_smooth_scales_in__=None, x_active_mask_in__=None, output_out_=None, ep_recv_count_out_=None, group_ep="", ep_rank_size=0, ep_rank_id=0, moe_expert_num=0, share_expert_num=0, share_expert_rank_num=0, quant_mode=0, global_bs=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, expert_ids_in__, gmm1_permuted_weight_in__, gmm1_permuted_weight_scale_in__, gmm2_weight_in__, gmm2_weight_scale_in__, expert_scales_in__, expert_smooth_scales_in__, x_active_mask_in__, output_out_, ep_recv_count_out_, group_ep, ep_rank_size, ep_rank_id, moe_expert_num, share_expert_num, share_expert_rank_num, quant_mode, global_bs)
    result = check_op_cap("get_op_specific_info", "DispatchGmmCombineDecode", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
