...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['topkIdx']
    output_names = ['numTokensPerRank', 'numTokensPerExpert', 'isTokenInRank']
...
def _build_args(topkIdx_in__, numTokensPerRank_out_, numTokensPerExpert_out_, isTokenInRank_out_, num_tokens, num_ranks, num_experts, num_topk):
    __inputs__ = []
    for arg in [topkIdx_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [numTokensPerRank_out_, numTokensPerExpert_out_, isTokenInRank_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if num_ranks != None:
        ...
        attr["name"] = "num_ranks"
        attr["dtype"] = "int"
        attr["value"] = num_ranks
        __attrs__.append(attr)
    if num_experts != None:
        attr = {}
        attr["name"] = "num_experts"
        attr["dtype"] = "int"
        attr["value"] = num_experts
        __attrs__.append(attr)
    if num_topk != None:
        attr = {}
        attr["name"] = "num_topk"
...
@tbe_register.register_operator("DispatchLayout", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def dispatch_layout(topkIdx_in__, numTokensPerRank_out_, numTokensPerExpert_out_, isTokenInRank_out_, num_tokens, num_ranks, num_experts, num_topk, kernel_name="dispatch_layout", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(topkIdx_in__, numTokensPerRank_out_, numTokensPerExpert_out_, isTokenInRank_out_, num_tokens, num_ranks, num_experts, num_topk)
    ...
    origin_func_name = "dispatch_layout"
    ascendc_src_dir_ex = "dispatch_layout"
    ascendc_src_dir = "dispatch_layout"
    ascendc_src_file = "dispatch_layout.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator DispatchLayout, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DispatchLayout"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[topkIdx_in__], origin_outputs = [numTokensPerRank_out_, numTokensPerExpert_out_, isTokenInRank_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(topkIdx_in__, numTokensPerRank_out_, numTokensPerExpert_out_, isTokenInRank_out_, num_tokens, num_ranks, num_experts, num_topk, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(topkIdx_in__, numTokensPerRank_out_, numTokensPerExpert_out_, isTokenInRank_out_, num_tokens, num_ranks, num_experts, num_topk)
    result = check_op_cap("op_select_format", "DispatchLayout", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(topkIdx_in__, numTokensPerRank_out_, numTokensPerExpert_out_, isTokenInRank_out_, num_tokens, num_ranks, num_experts, num_topk, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(topkIdx_in__, numTokensPerRank_out_, numTokensPerExpert_out_, isTokenInRank_out_, num_tokens, num_ranks, num_experts, num_topk)
    result = check_op_cap("get_op_specific_info", "DispatchLayout", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
