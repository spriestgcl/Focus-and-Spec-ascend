...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['recv_x', 'token_src_info', 'ep_recv_counts', 'recv_topk_weights', 'tp_recv_counts']
    output_names = ['x']
...
def _build_args(recv_x_in__, token_src_info_in__, ep_recv_counts_in__, recv_topk_weights_in__, tp_recv_counts_in__, x_out_, ep_group_name, ep_world_size, ep_rank_id, tp_group_name, tp_world_size, tp_rank_id, moe_expert_num, global_bs):
    __inputs__ = []
    for arg in [recv_x_in__, token_src_info_in__, ep_recv_counts_in__, recv_topk_weights_in__, tp_recv_counts_in__]:
        if arg != None:
    ...
    for arg in [x_out_]:
        if arg != None:
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if ep_group_name != None:
        attr = {}
        attr["name"] = "ep_group_name"
        attr["dtype"] = "str"
        attr["value"] = ep_group_name
    ...
    if ep_rank_id != None:
        ...
        __attrs__.append(attr)
    if tp_group_name != None:
        attr = {}
        attr["name"] = "tp_group_name"
        attr["dtype"] = "str"
        attr["value"] = tp_group_name
        __attrs__.append(attr)
...
@tbe_register.register_operator("MoeCombineNormal", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def moe_combine_normal(recv_x_in__, token_src_info_in__, ep_recv_counts_in__, recv_topk_weights_in__, tp_recv_counts_in__=None, x_out_=None, ep_group_name="", ep_world_size=0, ep_rank_id=0, tp_group_name="", tp_world_size=0, tp_rank_id=0, moe_expert_num=0, global_bs=0, kernel_name="moe_combine_normal", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(recv_x_in__, token_src_info_in__, ep_recv_counts_in__, recv_topk_weights_in__, tp_recv_counts_in__, x_out_, ep_group_name, ep_world_size, ep_rank_id, tp_group_name, tp_world_size, tp_rank_id, moe_expert_num, global_bs)
    ...
    origin_func_name = "moe_combine_normal"
    ascendc_src_dir_ex = "moe_combine_normal"
    ascendc_src_dir = "moe_combine_normal"
    ascendc_src_file = "moe_combine_normal.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator MoeCombineNormal, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeCombineNormal"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[recv_x_in__, token_src_info_in__, ep_recv_counts_in__, recv_topk_weights_in__, tp_recv_counts_in__], origin_outputs = [x_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0", "mc2_context_1"], param_type_list = ['required', 'required', 'required', 'required', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(recv_x_in__, token_src_info_in__, ep_recv_counts_in__, recv_topk_weights_in__, tp_recv_counts_in__=None, x_out_=None, ep_group_name="", ep_world_size=0, ep_rank_id=0, tp_group_name="", tp_world_size=0, tp_rank_id=0, moe_expert_num=0, global_bs=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(recv_x_in__, token_src_info_in__, ep_recv_counts_in__, recv_topk_weights_in__, tp_recv_counts_in__, x_out_, ep_group_name, ep_world_size, ep_rank_id, tp_group_name, tp_world_size, tp_rank_id, moe_expert_num, global_bs)
    result = check_op_cap("op_select_format", "MoeCombineNormal", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(recv_x_in__, token_src_info_in__, ep_recv_counts_in__, recv_topk_weights_in__, tp_recv_counts_in__=None, x_out_=None, ep_group_name="", ep_world_size=0, ep_rank_id=0, tp_group_name="", tp_world_size=0, tp_rank_id=0, moe_expert_num=0, global_bs=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(recv_x_in__, token_src_info_in__, ep_recv_counts_in__, recv_topk_weights_in__, tp_recv_counts_in__, x_out_, ep_group_name, ep_world_size, ep_rank_id, tp_group_name, tp_world_size, tp_rank_id, moe_expert_num, global_bs)
    result = check_op_cap("get_op_specific_info", "MoeCombineNormal", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
