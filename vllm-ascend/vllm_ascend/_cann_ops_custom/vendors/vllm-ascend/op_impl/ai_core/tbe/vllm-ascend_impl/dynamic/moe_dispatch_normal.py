...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'topk_idx', 'send_offset', 'send_tokenIdx', 'recv_offset', 'recv_count']
    output_names = ['recv_x', 'x_scales', 'assist_info_for_combine']
...
def _build_args(x_in__, topk_idx_in__, send_offset_in__, send_tokenIdx_in__, recv_offset_in__, recv_count_in__, recv_x_out_, x_scales_out_, assist_info_for_combine_out_, group_ep, ep_world_size, ep_rank_id, group_tp, tp_world_size, tp_rank_id, moe_expert_num, quant_mode, global_bs):
    __inputs__ = []
    for arg in [x_in__, topk_idx_in__, send_offset_in__, send_tokenIdx_in__, recv_offset_in__, recv_count_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [recv_x_out_, x_scales_out_, assist_info_for_combine_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if ep_world_size != None:
        attr = {}
        attr["name"] = "ep_world_size"
        attr["dtype"] = "int"
        attr["value"] = ep_world_size
        __attrs__.append(attr)
    if ep_rank_id != None:
        attr = {}
    ...
    if group_tp != None:
        ...
        attr["value"] = group_tp
        __attrs__.append(attr)
    if tp_world_size != None:
        attr = {}
        attr["name"] = "tp_world_size"
        attr["dtype"] = "int"
        attr["value"] = tp_world_size
        __attrs__.append(attr)
    if tp_rank_id != None:
        attr = {}
        attr["name"] = "tp_rank_id"
        attr["dtype"] = "int"
        attr["value"] = tp_rank_id
        __attrs__.append(attr)
...
@tbe_register.register_operator("MoeDispatchNormal", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def moe_dispatch_normal(x_in__, topk_idx_in__, send_offset_in__, send_tokenIdx_in__, recv_offset_in__, recv_count_in__, recv_x_out_, x_scales_out_, assist_info_for_combine_out_, group_ep, ep_world_size, ep_rank_id, group_tp="", tp_world_size=0, tp_rank_id=0, moe_expert_num=0, quant_mode=0, global_bs=0, kernel_name="moe_dispatch_normal", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, topk_idx_in__, send_offset_in__, send_tokenIdx_in__, recv_offset_in__, recv_count_in__, recv_x_out_, x_scales_out_, assist_info_for_combine_out_, group_ep, ep_world_size, ep_rank_id, group_tp, tp_world_size, tp_rank_id, moe_expert_num, quant_mode, global_bs)
    ...
    origin_func_name = "moe_dispatch_normal"
    ascendc_src_dir_ex = "moe_dispatch_normal"
    ascendc_src_dir = "moe_dispatch_normal"
    ascendc_src_file = "moe_dispatch_normal.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator MoeDispatchNormal, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeDispatchNormal"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, topk_idx_in__, send_offset_in__, send_tokenIdx_in__, recv_offset_in__, recv_count_in__], origin_outputs = [recv_x_out_, x_scales_out_, assist_info_for_combine_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0", "mc2_context_1"], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(x_in__, topk_idx_in__, send_offset_in__, send_tokenIdx_in__, recv_offset_in__, recv_count_in__, recv_x_out_, x_scales_out_, assist_info_for_combine_out_, group_ep, ep_world_size, ep_rank_id, group_tp="", tp_world_size=0, tp_rank_id=0, moe_expert_num=0, quant_mode=0, global_bs=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, topk_idx_in__, send_offset_in__, send_tokenIdx_in__, recv_offset_in__, recv_count_in__, recv_x_out_, x_scales_out_, assist_info_for_combine_out_, group_ep, ep_world_size, ep_rank_id, group_tp, tp_world_size, tp_rank_id, moe_expert_num, quant_mode, global_bs)
    result = check_op_cap("op_select_format", "MoeDispatchNormal", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, topk_idx_in__, send_offset_in__, send_tokenIdx_in__, recv_offset_in__, recv_count_in__, recv_x_out_, x_scales_out_, assist_info_for_combine_out_, group_ep, ep_world_size, ep_rank_id, group_tp="", tp_world_size=0, tp_rank_id=0, moe_expert_num=0, quant_mode=0, global_bs=0, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, topk_idx_in__, send_offset_in__, send_tokenIdx_in__, recv_offset_in__, recv_count_in__, recv_x_out_, x_scales_out_, assist_info_for_combine_out_, group_ep, ep_world_size, ep_rank_id, group_tp, tp_world_size, tp_rank_id, moe_expert_num, quant_mode, global_bs)
    result = check_op_cap("get_op_specific_info", "MoeDispatchNormal", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
