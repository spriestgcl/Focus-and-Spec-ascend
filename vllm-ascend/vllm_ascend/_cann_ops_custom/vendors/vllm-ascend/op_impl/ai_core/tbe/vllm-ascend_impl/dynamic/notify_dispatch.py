...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['sendData', 'tokenPerExpertData']
    output_names = ['sendDataOffset', 'recvData']
...
def _build_args(sendData_in__, tokenPerExpertData_in__, sendDataOffset_out_, recvData_out_, sendCount, num_tokens, comm_group, rank_size, rank_id, local_rank_size, local_rank_id):
    __inputs__ = []
    for arg in [sendData_in__, tokenPerExpertData_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [sendDataOffset_out_, recvData_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if sendCount != None:
        ...
        attr["name"] = "sendCount"
        attr["dtype"] = "int"
        attr["value"] = sendCount
        __attrs__.append(attr)
    if num_tokens != None:
        attr = {}
        attr["name"] = "num_tokens"
        attr["dtype"] = "int"
        attr["value"] = num_tokens
        __attrs__.append(attr)
    if comm_group != None:
        attr = {}
        attr["name"] = "comm_group"
        attr["dtype"] = "str"
        attr["value"] = comm_group
        __attrs__.append(attr)
    if rank_size != None:
        attr = {}
        attr["name"] = "rank_size"
        attr["dtype"] = "int"
        attr["value"] = rank_size
        __attrs__.append(attr)
    if rank_id != None:
        attr = {}
        attr["name"] = "rank_id"
        ...
        attr["value"] = rank_id
        __attrs__.append(attr)
    if local_rank_size != None:
        attr = {}
        attr["name"] = "local_rank_size"
        attr["dtype"] = "int"
        attr["value"] = local_rank_size
        __attrs__.append(attr)
    if local_rank_id != None:
        attr = {}
        attr["name"] = "local_rank_id"
        attr["dtype"] = "int"
        attr["value"] = local_rank_id
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("NotifyDispatch", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def notify_dispatch(sendData_in__, tokenPerExpertData_in__, sendDataOffset_out_, recvData_out_, sendCount, num_tokens, comm_group, rank_size, rank_id, local_rank_size, local_rank_id, kernel_name="notify_dispatch", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(sendData_in__, tokenPerExpertData_in__, sendDataOffset_out_, recvData_out_, sendCount, num_tokens, comm_group, rank_size, rank_id, local_rank_size, local_rank_id)
    ...
    origin_func_name = "notify_dispatch"
    ascendc_src_dir_ex = "notify_dispatch"
    ascendc_src_dir = "notify_dispatch"
    ascendc_src_file = "notify_dispatch.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator NotifyDispatch, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "NotifyDispatch"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[sendData_in__, tokenPerExpertData_in__], origin_outputs = [sendDataOffset_out_, recvData_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0"], param_type_list = ['required', 'required', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(sendData_in__, tokenPerExpertData_in__, sendDataOffset_out_, recvData_out_, sendCount, num_tokens, comm_group, rank_size, rank_id, local_rank_size, local_rank_id, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(sendData_in__, tokenPerExpertData_in__, sendDataOffset_out_, recvData_out_, sendCount, num_tokens, comm_group, rank_size, rank_id, local_rank_size, local_rank_id)
    result = check_op_cap("op_select_format", "NotifyDispatch", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(sendData_in__, tokenPerExpertData_in__, sendDataOffset_out_, recvData_out_, sendCount, num_tokens, comm_group, rank_size, rank_id, local_rank_size, local_rank_id, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(sendData_in__, tokenPerExpertData_in__, sendDataOffset_out_, recvData_out_, sendCount, num_tokens, comm_group, rank_size, rank_id, local_rank_size, local_rank_id)
    result = check_op_cap("get_op_specific_info", "NotifyDispatch", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
