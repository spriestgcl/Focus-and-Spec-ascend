...
class NPUPlatform(Platform):
    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        if model_config is not None and not model_config.use_mla:
            ...
            scheduler_delay_factor = getattr(scheduler_config,
                                             "delay_factor", 0)
            scheduler_send_delta_data = getattr(scheduler_config,
                                                "send_delta_data", False)
            scheduler_num_steps = getattr(scheduler_config,
                                          "num_scheduler_steps", 1)
            if envs.VLLM_USE_V1 and \
                not model_config.is_multimodal_model and \
                (decoding_config is None or getattr(decoding_config, "backend", "auto") == "auto") and \
                not scheduler_delay_factor > 0 and \
                not scheduler_send_delta_data and \
