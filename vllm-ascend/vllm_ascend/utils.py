...
try:
    ...
    logger.warning("Torchair is unavailable, fallback to eager-compatible stubs: %s", e)

    def _super_kernel(prefix: str, stream: str):
        return nullcontext()

    def _npu_stream_switch(tag: str, priority: int):
        return nullcontext()
...
def update_aclgraph_sizes(vllm_config: VllmConfig) -> None:
    ...
    # vLLM 0.11 may leave capture sizes unset and derive them later. In that
    # case there is nothing to resize here, so keep the config untouched.
