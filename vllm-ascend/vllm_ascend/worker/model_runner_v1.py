...
try:
    ...
    torchair = None  # type: ignore[assignment]

    def patch_for_hcom() -> None:
        return None

    _logger.warning(
        "Torchair import failed in model_runner_v1; graph compile path will "
        "be disabled unless torchair is available: %s", e)
...
try:
    ...
    logger.warning(
        "FusedMoE import failed; MoE-specific branches will be disabled: %s",
        e)

    class FusedMoE(nn.Module):  # type: ignore[no-redef]
        pass
...
try:
    ...
    logger.warning(
        "EagleProposer import failed; eagle speculative decoding will be "
        "disabled: %s", e)

    class EagleProposer:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise RuntimeError(
                "Eagle speculative decoding is unavailable in this runtime.")
...
try:
    ...
    def is_spec_decode_supported(req_id, input_batch):  # type: ignore[no-redef]
        req_idx = input_batch.req_id_to_index.get(req_id)
        if req_idx is None:
            return True
        sampling_params = input_batch.sampling_params[req_idx]
        if sampling_params is None:
            return True
        return not is_spec_decode_unsupported(sampling_params)
...
class NPUModelRunner(LoRAModelRunnerMixin):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        if self.torchair_graph_enabled and torchair is None:
            raise RuntimeError(
                "torchair_graph_config.enabled=True but torchair is unavailable. "
                "Disable torchair graph mode or fix torchair dependencies.")
