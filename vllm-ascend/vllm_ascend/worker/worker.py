...
class NPUWorker(LocalOrDistributedWorkerBase):
    ...
    def _is_sleep_mode_effective(self) -> bool:
        """Sleep mode requires vllm_ascend_C (camem) extension support."""
        enabled = self.vllm_config.model_config.enable_sleep_mode
        if enabled and not camem_available:
            logger.warning("camem allocator is unavailable; "
                           "disabling sleep mode fallback.")
            self.vllm_config.model_config.enable_sleep_mode = False
            return False
        return enabled

    def sleep(self, level: int = 1) -> None:
        if not self._is_sleep_mode_effective():
            logger.warning("Sleep requested but camem allocator is unavailable; "
                           "skip sleep.")
            return
    ...
    def wake_up(self, tags: Optional[list[str]] = None) -> None:
        if not self._is_sleep_mode_effective():
            logger.warning("Wake up requested but camem allocator is unavailable; "
                           "skip wake up.")
            return
        allocator = CaMemAllocator.get_instance()
    ...
    def load_model(self):
        if self._is_sleep_mode_effective():
            allocator = CaMemAllocator.get_instance()
